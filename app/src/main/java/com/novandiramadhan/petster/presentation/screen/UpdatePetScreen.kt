...
import com.novandiramadhan.petster.presentation.viewmodel.UpdatePetViewModel

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun UpdatePetScreen(
    viewModel: UpdatePetViewModel = hiltViewModel(),
    petId: String,
    back: () -> Unit,
) {
    ...
    when (uploadState) {
        is UploadState.Loading -> {
            LoadingDialog(
                title =  stringResource(R.string.update_pet_loading)
            )
        }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(R.string.update_pet)
                    )
                },
                navigationIcon = {
                    IconButton(
...
@Composable
private fun UpdatePetContent(
    viewModel: UpdatePetViewModel = hiltViewModel()
) {
    val petState = viewModel.updatedPet.collectAsState().value
    ...
    Column(
        ...
        PetPhotosForm(
            petImage = petState?.image,
            photoUris = photoUris,
            modal = modal,
            imageCropper = viewModel.imageCropper,
            onAddPhoto = viewModel::addPhoto,
            onUpdatePhoto = viewModel::updatePhotoUri,
            onUpdateUrlPhoto = viewModel::updateUrlPhoto,
            onRemovePhoto = viewModel::removePhoto,
            onRemoveUrlPhoto = viewModel::removeUrlPhoto,
            onSetCoverPhoto = viewModel::setCoverPhoto,
            onSetUrlPhotoAsCover = viewModel::setUrlPhotoAsCover,
        ...
        DropdownField(
            ...
            selectedOption = stringArrayResource(R.array.confirm)[
                if (petState?.isVaccinated == true) 1 else 0
            ]
        )
        ...
        TextFormFieldRupiah(
            ...
            value = petState?.adoptionFee?.toString() ?: "",
